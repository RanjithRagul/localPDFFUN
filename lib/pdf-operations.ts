...
export async function encryptPDF(
  ...
  try {
    // Strategy 1: Clean Slate (Create New + Copy Pages)
    // This allows pdf-lib to generate fresh encryption dictionaries, which is more reliable
    // than modifying an existing document in certain browser environments.
    ...
    newDoc.setTitle(srcDoc.getTitle() || file.name);
    newDoc.setProducer('PDF0 Secure');
    newDoc.setCreator('PDF0 Secure');

    const ownerPwd = options.ownerPassword || options.userPassword;

    // Save with standard encryption
    let encryptedBytes = await newDoc.save({
      userPassword: options.userPassword,
      ownerPassword: ownerPwd,
      // Note: We skip setting complex permissions initially to maximize compatibility
      // The prompt suggests encryption failed silently with complex setups in some contexts.
    });

    // VERIFICATION STEP
    // Try to open the file without a password. If it opens, encryption failed.
    let isLocked = false;
    try {
        await PDFDocument.load(encryptedBytes);
        isLocked = false; // Opened successfully without password -> FAILURE
    } catch (e) {
        isLocked = true; // Failed to open -> SUCCESS
    }

    if (!isLocked) {
        console.warn("Primary encryption strategy failed. Attempting fallback strategy...");
        
        // Strategy 2: Direct Save
        // Sometimes reloading the original and saving directly works better
        const directDoc = await PDFDocument.load(arrayBuffer, { ignoreEncryption: true });
        encryptedBytes = await directDoc.save({
            userPassword: options.userPassword,
            ownerPassword: ownerPwd,
        });
        ...
        try {
            await PDFDocument.load(encryptedBytes);
            throw new Error("Encryption failed: The file could not be locked.");
        } catch (e: any) {
            if (e.message && e.message.includes("Encryption failed")) throw e;
            // Success
        }
    }
